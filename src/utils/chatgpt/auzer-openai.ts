...
export class AuzerChatGptAPI {
  ...
  async sendMessage(
    ...
    const responseP = new Promise<types.ChatMessage>(
      ...
      async (resolve, reject) => {
        const url = this._apiVersion ? `${this._apiBaseUrl}/chat/completions?api-version=${this._apiVersion}` : `${this._apiBaseUrl}/chat/completions`
        const headers = {
          ...
          fetchSSE(
            url,
            {
              ...
              onMessage: (data: string) => {
                globalThis.console.log('OpenAI stream SEE event', JSON.parse(data).choices[0]?.finish_reason)
                const finish_reason = ['content_filter', 'stop', 'length']
                ...
                if (finish_reason.includes(JSON.parse(data).choices[0]?.finish_reason)) {
                  globalThis.console.log('OpenAI stream SEE event finish_reason', JSON.parse(data).choices[0].finish_reason)
                  result.text = result.text.trim()
                  return resolve(result)
                }
  ...
  protected async _buildMessages(text: string, opts: types.SendMessageOptions) {
    ...
    messages = processArray(messages)
...
function processArray(l: any[]): any[] {
  if (l.length <= 8) {
    return l // 返回原数组
  }
  else if (l.length > 8) {
    const firstThree = l.slice(0, 3) // 取前三个元素
    const lastFive = l.slice(-5) // 取后五个元素
    return firstThree.concat(lastFive) // 连接前三个元素和后五个元素
  }
}
